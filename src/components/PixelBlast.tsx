...
const resolveCssColor = (value: string, container: HTMLElement): string => {
  ...
  context.fillStyle = computedColor || value
...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  useEffect(() => {
    const updateThemeColor = () => {
      ...
      scene.uniforms.uColor.value.set(resolveCssColor(color, container))
      if (scene.composer) scene.composer.render()
      else scene.renderer.render(scene.scene, scene.camera)
    }

    updateThemeColor()
    window.addEventListener('theme-change', updateThemeColor)
    window.addEventListener('primary-color-change', updateThemeColor)
    return () => {
      window.removeEventListener('theme-change', updateThemeColor)
      window.removeEventListener('primary-color-change', updateThemeColor)
    }
  }, [color])
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      const uniforms = {
        ...
        uTime: { value: 0 },
        uColor: { value: new THREE.Color(resolveCssColor(color, container)) },
        uClickPos: {
          value: Array.from(
            { length: MAX_CLICKS },
      ...
      t.uniforms.uColor.value.set(resolveCssColor(color, container))
