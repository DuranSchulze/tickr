...
export function WorkspaceSubscriptionGate({
  ...
  return (
    <main className="relative grid min-h-0 flex-1 place-items-center overflow-y-auto bg-eggshell p-5">
      <div
        aria-hidden="true"
      ...
      />
      <section className="relative w-full max-w-2xl border border-stone bg-eggshell p-6 shadow-[8px_8px_0_color-mix(in_oklab,var(--border)_70%,transparent)] sm:p-9">
        <span className="inline-flex size-11 items-center justify-center border border-primary bg-primary/10 text-primary">
          <LockKeyhole className="size-5" aria-hidden="true" />
        </span>
        <p className="mt-6 text-xs font-black uppercase tracking-[0.16em] text-graphite">
        ...
        {isOwner ? (
          <Link
            ...
            className="mt-7 inline-flex min-h-11 items-center gap-2 border border-primary bg-primary-action px-5 text-sm font-black text-primary-action-foreground no-underline"
          >
